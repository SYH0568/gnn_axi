...
example_node_attrgXb_x::~example_node_attrgXb_x() {
...
void example_node_attrgXb_x::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void example_node_attrgXb_x::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void example_node_attrgXb_x::proc_i_q0() {
    i_q0.write(buf_q0[prev_iptr.read()].read());
}

void example_node_attrgXb_x::proc_t_q0() {
    t_q0.write(buf_q0[prev_tptr.read()].read());
}

void example_node_attrgXb_x::proc_buffer_signals() {
    for (unsigned i = 0; i < BufferCount; i++) {
        if (iptr.read() == i) {
...
void example_node_attrgXb_x::proc_push_buf() {
    push_buf.write(i_ce.read() & i_write.read() & full_n.read());
}

void example_node_attrgXb_x::proc_pop_buf() {
    pop_buf.write(t_ce.read() & t_read.read() & empty_n.read());
}

void example_node_attrgXb_x::proc_iptr() {
...
void example_node_attrgXb_x::proc_tptr() {
    if (reset.read() == SC_LOGIC_1) {
        tptr.write(0);
        prev_tptr.write(0);
...
void example_node_attrgXb_x::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void example_node_attrgXb_x::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void example_node_attrgXb_x::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
