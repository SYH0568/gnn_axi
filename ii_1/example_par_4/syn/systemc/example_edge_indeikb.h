...
#ifndef _example_edge_indeikb_H_
#define _example_edge_indeikb_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "example_edge_indeikb_memcore.h"

SC_MODULE(example_edge_indeikb) {
...
protected:
    ...
    // buffer instances
    example_edge_indeikb_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(example_edge_indeikb) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "example_edge_indeikb_memcore_U_";
            name += szTmp;

            buffer[i] = new example_edge_indeikb_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "example_edge_indeikb.trace");
            // ports
