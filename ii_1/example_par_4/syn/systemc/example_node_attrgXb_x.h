...
#ifndef _example_node_attrgXb_x_H_
#define _example_node_attrgXb_x_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "example_node_attrgXb_x_memcore.h"

SC_MODULE(example_node_attrgXb_x) {
...
protected:
    ...
    // buffer instances
    example_node_attrgXb_x_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(example_node_attrgXb_x) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "example_node_attrgXb_x_memcore_U_";
            name += szTmp;

            buffer[i] = new example_node_attrgXb_x_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "example_node_attrgXb_x.trace");
            // ports
